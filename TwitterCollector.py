    ...
    def on_data(self, data):
        try:
            ...
            if self._translate:
                ...
                tweet_translated = str(tweet_original)
            created_at = datetime.datetime.utcfromtimestamp(int(data['timestamp_ms'])/1000).strftime('%Y-%m-%dT%H:%M:%SZ')
            ...
            save_sentiment(self._search_query, self._end_time,
                           twitter_id, tweet_original, tweet_translated, lang, created_at, geo, reply_count,
                           retweet_count, likes_count, user_id, user_name, user_location, user_description,
                           user_verified, user_followers_count, user_friends_count, polarity, subjectivity,
                          )
